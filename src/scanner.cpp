...
Token* Scanner::nextToken() {
    Token* token;
    while (current < static_cast<int>(input.length()) &&  is_white_space(input[current]) ) current++;
    if (current >= static_cast<int>(input.length())) return new Token(Token::END);
    char c  = input[current];
    first = current;
    if (isdigit(c)) {
        current++;
        while (current < static_cast<int>(input.length()) && isdigit(input[current]))
            current++;
        ...
        current++;
        while (current < static_cast<int>(input.length()) && isalnum(input[current]))
            current++;
        ...
        switch(c) {
            ...
            case '=':
                if (current + 1 < static_cast<int>(input.length()) && input[current + 1] == '=') {
