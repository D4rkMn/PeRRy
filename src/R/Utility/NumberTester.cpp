...
bool R::NumberTester::hasTraillingWhitespace(const string& str) {
...
bool R::NumberTester::isNumber(const string& numStr) {
...
bool R::NumberTester::fitsInInt32(const string& numStr) {
...
bool R::NumberTester::fitsInUInt32(const string& numStr) {
    if (!NumberTester::isNumber(numStr)) {
        return false;
    }
...
bool R::NumberTester::fitsInInt64(const string& numStr) {
    if (!NumberTester::isNumber(numStr)) {
        return false;
    }
    if (numStr.find('.') != string::npos) {
...
bool R::NumberTester::fitsInUInt64(const string& numStr) {
    if (!NumberTester::isNumber(numStr)) {
        return false;
    }
    if (!numStr.empty() && numStr[0] == '-') {
