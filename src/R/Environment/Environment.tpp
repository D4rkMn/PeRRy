...
template <typename T>
int R::Environment<T>::searchRib(const string& var) const {
    int idx = ribs.size() - 1;
    while (idx >= 0) {
...
template <typename T>
void R::Environment<T>::addLevel() {
    unordered_map<string, T> r;
    ribs.push_back(r);
}

template <typename T>
void R::Environment<T>::clear() {
    ribs.clear();
}

template <typename T>
void R::Environment<T>::addVariable(const string& var, const T& value) {
    if (ribs.size() == 0) {
...
template <typename T>
void R::Environment<T>::addVariable(const string& var) {
    ribs.back()[var] = T{0};
}

template <typename T>
bool R::Environment<T>::removeLevel() {
    if (ribs.size() > 0) {
        ribs.pop_back();
        return true;
    }
...
template <typename T>
bool R::Environment<T>::updateVariableValue(const string& x, const T& v) {
    int idx = searchRib(x);
    if (idx < 0) {
        return false;
    }
...
template <typename T>
optional<T> R::Environment<T>::getVariableValue(const string& x) const {
    int idx = searchRib(x);
    if (idx < 0) {
        return nullopt;
    }
