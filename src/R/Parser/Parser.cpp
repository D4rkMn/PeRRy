...
void R::Parser::resetScanner() { scanner = nullptr; }

void R::Parser::setScanner(Scanner* s) { 
    scanner = s;
    previous = nullptr;
...
bool R::Parser::match(Token::Type ttype) {
    if (check(ttype)) {
        advance();
        return true;
    }
...
bool R::Parser::check(Token::Type ttype) {
    if (isAtEnd()) return false;
    return (current->type == ttype);
}

bool R::Parser::matchVarType() {
    if (checkVarType()) {
        advance();
        return true;
    }
...
bool R::Parser::isAtEnd() {
    return (current->type == Token::END);
}

void R::Parser::backtrack() {
    scanner->backtrack();
    current = previous;
    previous = nullptr;
}

runtime_error R::Parser::SyntaxError(const string& msg) const {
    string s = "Error: " + msg + ", pero en lugar se encontró '" + current->text + "' - línea " + to_string(current->line);
    return runtime_error(s);
}

R::VarType R::Parser::tokenTypeToVarType(Token::Type type) const {
    switch (type) {
...
R::BinaryOp R::Parser::tokenTypeToBinaryOp(Token::Type type) const {
    switch (type) {
